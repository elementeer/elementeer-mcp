...
export function registerCharityTools(
  ...
  // ------------------------------------------------------------------ //
  // wizard_charity (CHARITY-003)
  // ------------------------------------------------------------------ //
  server.tool(
    'wizard_charity',
    'Charity module wizard — Fundraising setup, donation form optimization, payment gateway tips.',
    {
      site_id: z.string().optional().describe('Site ID from config (defaults to active site)'),
    },
    async ({ site_id }) => {
      try {
        const client = getClient(site_id);
        const result = await client.getWizard('charity');
