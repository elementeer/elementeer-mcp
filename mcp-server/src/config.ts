...
export function getIntegrations(): { pexels_api_key?: string; unsplash_access_key?: string } {
  try {
    const config = loadConfig();
    return (config as { integrations?: { pexels_api_key?: string; unsplash_access_key?: string } }).integrations ?? {};
  } catch {
    return {};
  }
}
