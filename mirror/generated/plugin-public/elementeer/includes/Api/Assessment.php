 ...
 *
 * Each collector runs in isolation: a fatal error in one collector does not
 * block the entire response. The errored collector is reported under
 * `collector_errors` with file, line, and message.
 */
final class Assessment {
    ...
    public function get_assessment( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        $run = function ( string $collector, callable $fn ) use ( &$issues, &$collector_errors ) {
            try {
                $result = $fn();
                if ( $result !== null ) {
                    return $result;
                }
                return []; // defensive — never return null to destructuring
            } catch ( \Throwable $e ) {
                $collector_errors[] = [
                    'collector' => $collector,
                    ...
                    'file'      => $e->getFile(),
                    'line'      => $e->getLine(),
                    'trace_id'  => substr( wp_generate_uuid4(), 0, 8 ),
                ];
                // Also write to WP debug log
                error_log( sprintf(
                    '[Elementeer Assessment] %s collector failed: %s in %s:%d',
                    $collector, $e->getMessage(), $e->getFile(), $e->getLine()
                ) );
                return null;
            }
        };
        ...
        // ── 1. WordPress basics ──────────────────────────────────────────
        $wordpress = $run( 'wordpress', function () {
            return [
        ...
        // ── 2. Elementor status ──────────────────────────────────────────
        $elementor = $run( 'elementor', function () {
        ...
        // Elementor-related issues (computed outside collector for readability)
        if ( $elementor !== null && empty( $elementor['version'] ) ) {
            $issues[] = [ 'severity' => 'critical', 'code' => 'elementor_not_active', 'message' => 'Elementor plugin is not active.' ];
        }

        // ── 3. Brand / Global Styles ─────────────────────────────────────
        $brand = $run( 'brand', function () use ( &$issues ) {
            $kit_id = (int) \get_option( 'elementor_active_kit', 0 );
        ...
        $template_library = $run( 'template_library', function () use ( &$issues ) {
        ...
        $performance = $run( 'performance', function () use ( &$issues ) {
        ...
        // ── 10. Booking gaps (optional) ──────────────────────────────────
        $booking_recommendations = [];
        if ( $plugins !== null && ! empty( $plugins['classified']['booking'] ?? null ) ) {
            $run( 'booking_recommendations', function () use ( &$issues, &$booking_recommendations ) {
                $wizard_assessment = [
                    'plugins' => $plugins ?? [],
                    'pages'   => $pages ?? [],
                ];
        ...
        // ── 11. Capabilities ─────────────────────────────────────────────
        $capabilities = $run( 'capabilities', function () {
            $plugins_classified = $plugins['classified'] ?? [];
            $seo_plugin = null;
        ...
        $custom_post_types = $run( 'custom_post_types', function () {
            $cpt_objects = \get_post_types( [ 'public' => true, '_builtin' => false ], 'objects' );
            return array_values( array_map( function ( $cpt ) {
                return [
                    'name'  => $cpt->name,
                    'label' => $cpt->label,
        ...
        // ── 13. User roles ───────────────────────────────────────────────
        $user_roles = $run( 'user_roles', function () {
        ...
        $status = empty( $collector_errors ) ? 200 : 500;
