...
echo "  • Enthaltene Verzeichnisse (Auszug):"
find "$PLUGIN_SLUG" -type d | head -20 | while read dir; do
    echo "    📁 $dir"
done
...
# Prüfe auf leere Ordner im ZIP
echo ""
echo "  • Prüfe auf leere Ordner im ZIP:"
EMPTY_DIRS=0
find "$PLUGIN_SLUG" -type d -empty 2>/dev/null | while read dir; do
    echo "    ⚠️  Leerer Ordner: $dir"
    EMPTY_DIRS=$((EMPTY_DIRS + 1))
done

if [ "$EMPTY_DIRS" -eq 0 ]; then
    echo "    ✅ Keine leeren Ordner im ZIP"
else
    echo "    ⚠️  $EMPTY_DIRS leere Ordner gefunden (normal für assets/css/js/fonts wenn leer)"
fi
...
echo "   keine leeren Ordner wie css/, js/, fonts/ wenn sie leer sind."
echo ""
