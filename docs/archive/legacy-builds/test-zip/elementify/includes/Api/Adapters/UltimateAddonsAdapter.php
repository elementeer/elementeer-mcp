...
/**
 * Adapter for Ultimate Addons for Elementor (Free & Pro).
 */
final class UltimateAddonsAdapter extends BaseAddonAdapter {

	/**
	 * Plugin basename for the free version.
	 */
	private const FREE_BASENAME = 'ultimate-elementor/ultimate-elementor.php';

	/**
	 * Plugin basename for the pro version.
	 */
	private const PRO_BASENAME = 'ultimate-elementor-pro/ultimate-elementor-pro.php';

	/**
	 * Option name where widget activation status is stored.
	 */
	private const OPTION_NAME = 'uael_save_settings';
	...
	protected function detect_tier(): ?string {
		if ( defined( 'UAEL_PRO_PLUGIN_BASENAME' ) ) {
			return 'pro';
		}
		if ( defined( 'UAEL_PLUGIN_BASENAME' ) ) {
			return 'free';
		}
	...
	/**
	 * Retrieve the list of UAE widgets with activation status.
	 *
	 * @inheritDoc
	 */
	...
	public function get_capabilities(): array {
		$caps = [];
		if ( $this->active ) {
			$caps[] = 'ultimate-addons-widgets';
			$caps[] = 'uael-' . $this->detect_tier();
		}
		return $caps;
	}
}