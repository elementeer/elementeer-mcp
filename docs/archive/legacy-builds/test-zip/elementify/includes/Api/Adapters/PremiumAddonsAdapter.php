...
/**
 * Adapter for Premium Addons for Elementor (Free & Pro).
 */
final class PremiumAddonsAdapter extends BaseAddonAdapter {

	/**
	 * Plugin basename for the free version.
	 */
	private const FREE_BASENAME = 'premium-addons-for-elementor/premium-addons-for-elementor.php';

	/**
	 * Plugin basename for the pro version.
	 */
	private const PRO_BASENAME = 'premium-addons-pro/premium-addons-for-elementor.php';

	/**
	 * Option name where widget activation status is stored.
	 */
	private const OPTION_NAME = 'pa_save_settings';
	...
	/**
	 * Retrieve the list of Premium Addons widgets with activation status.
	 *
	 * @inheritDoc
	 */
	...
	public function get_capabilities(): array {
		$caps = [];
		if ( $this->active ) {
			$caps[] = 'premium-addons-widgets';
			$caps[] = 'premium-' . $this->detect_tier();
		}
		return $caps;
	}
}