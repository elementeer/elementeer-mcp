...
/**
 * Adapter for Essential Addons for Elementor (Free & Pro).
 */
final class EssentialAddonsAdapter extends BaseAddonAdapter {

	/**
	 * Plugin basename for the free version.
	 */
	private const FREE_BASENAME = 'essential-addons-for-elementor-lite/essential_adons_elementor.php';

	/**
	 * Plugin basename for the pro version.
	 */
	private const PRO_BASENAME = 'essential-addons-elementor/essential_adons_elementor.php';

	/**
	 * Option name where widget activation status is stored.
	 */
	private const OPTION_NAME = 'eael_save_settings';
	...
	protected function detect_tier(): ?string {
		...
		if ( defined( 'EAEL_PLUGIN_BASENAME' ) ) {
			return 'free';
		}
	...
	/**
	 * Retrieve the list of EA widgets with activation status.
	 *
	 * @inheritDoc
	 */
	...
	public function get_capabilities(): array {
		$caps = [];
		if ( $this->active ) {
			$caps[] = 'essential-addons-widgets';
			$caps[] = 'eael-' . $this->detect_tier();
		}
		return $caps;
	}
}