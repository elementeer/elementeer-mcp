...
/**
 * Adapter for The Plus Addons for Elementor (Free & Pro).
 */
final class ThePlusAddonsAdapter extends BaseAddonAdapter {

	/**
	 * Plugin basename for the free version.
	 */
	private const FREE_BASENAME = 'the-plus-addons-for-elementor-page-builder/the-plus-addons-for-elementor-page-builder.php';

	/**
	 * Plugin basename for the pro version.
	 */
	private const PRO_BASENAME = 'the-plus-addons-for-elementor-page-builder-pro/the-plus-addons-for-elementor-page-builder-pro.php';

	/**
	 * Option name where widget activation status is stored.
	 */
	private const OPTION_NAME = 'theplus_elementor_options';
	...
	protected function detect_tier(): ?string {
		if ( defined( 'THEPLUS_ELEMENTOR_PRO_VERSION' ) ) {
			return 'pro';
		}
		if ( defined( 'THEPLUS_ELEMENTOR_VERSION' ) ) {
			return 'free';
		}
	...
	/**
	 * Retrieve the list of The Plus Addons widgets with activation status.
	 *
	 * @inheritDoc
	 */
	...
	public function get_capabilities(): array {
		$caps = [];
		if ( $this->active ) {
			$caps[] = 'the-plus-addons-widgets';
			$caps[] = 'tp-' . $this->detect_tier();
		}
		return $caps;
	}
}